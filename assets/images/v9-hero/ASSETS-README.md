...
### Scène 1 — Pod éveil (`scene-1-pod-eveil-16x9`)
...
**Pas générées pour l'instant.** Stratégie actuelle = recadrage CSS via `object-fit: cover` + `object-position` calibré (cf `assets/css/mobile-focal-points.css` du repo). Si jamais la qualité du recadrage CSS est insuffisante en review mobile, regénérer en 9:16 dédié (aspect ratio 9:16 sur Higgsfield, ~12 crédits pour les 6).

---

## Fichiers binaires attendus dans ce dossier

Les 6 paires `.png` + `.webp` listées dans `CURSOR-V9-HERO-BRIEF.md` §3 doivent être poussées ici depuis la machine où elles ont été exportées (non versionnées dans ce commit si absentes).
