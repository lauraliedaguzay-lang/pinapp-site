...
const PandoraNight = {
  ...
  active: false, _scale: 1,

  /* ── 6 grandes nappes bioluminescentes ─────────────────── */
  layers: [
    { cx:0.15, cy:0.65, rx:0.42, ry:0.32, r:0,   g:160, b:210, a:0.32, vx:0.00018, vy:0.00014, pw:0.00008, pa:0.06 },
    { cx:0.50, cy:0.40, rx:0.38, ry:0.28, r:0,   g:210, b:170, a:0.28, vx:-0.00015,vy:0.00012, pw:0.00010, pa:0.05 },
    { cx:0.80, cy:0.28, rx:0.40, ry:0.30, r:90,  g:10,  b:150, a:0.22, vx:0.00012, vy:-0.00016,pw:0.00006, pa:0.04 },
    { cx:0.22, cy:0.20, rx:0.32, ry:0.24, r:140, g:40,  b:220, a:0.18, vx:-0.00010,vy:0.00012, pw:0.00007, pa:0.03 },
    { cx:0.88, cy:0.72, rx:0.20, ry:0.15, r:255, g:180, b:40,  a:0.14, vx:0.00008, vy:-0.00010,pw:0.00012, pa:0.04 },
    { cx:0.45, cy:0.88, rx:0.28, ry:0.20, r:0,   g:200, b:120, a:0.16, vx:0.00014, vy:0.00008, pw:0.00009, pa:0.05 },
  ],
  ...
  _buildParticles(isMobile) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const bx = 0.05 + Math.random() * 0.90;
      const by = 0.10 + Math.random() * 0.90;
      this.particles.push({
        bx, by,
        ...
        drift: -(0.000012 + Math.random() * 0.000020),
  ...
  init() {
    ...
    document.addEventListener('visibilitychange', () => {
      ...
      else { this.active = true; this.draw(); }
    });
  ...
  draw() {
    ...
    this.layers.forEach(l => {
      const x = W * (l.cx + Math.sin(this.t * l.vx * 1000) * 0.12 + Math.cos(this.t * l.vx * 700) * 0.05);
      const y = H * (l.cy + Math.cos(this.t * l.vy * 1000) * 0.10 + Math.sin(this.t * l.vy * 600) * 0.04);
    ...
    this.particles.forEach(p => {
      ...
      const cy = p.by + Math.sin(this.t * p.fy + p.py) * p.Ay
               + p.drift * p.life;
      const px = cx * W;
      const py = cy * H;
      ...
      const env = lifeRatio < 0.08 ? lifeRatio / 0.08
                : lifeRatio > 0.88 ? (1 - lifeRatio) / 0.12
                : 1.0;
  ...
  destroy() {
    ...
    this.canvas = null; this.active = false;
  }
};
