 ...
 *
 * IMPORTANT: ce fichier doit fonctionner sans <script type="module">.
 */
...
(function () {
  ...
  function getProgress() {
    const raw = getLocal('pinapp-academy-progress');
    if (!raw) {
      return {
        done: {},
        score: {},
    ...
    // PinappStorage.get() peut déjà parser JSON
    return raw;
  }
  ...
  function wireLegacyAcademy01() {
    const build = QS('#academyBuildQa');
    const copy = QS('#academyCopyQa');
    const out = QS('#academyQaOut');
    if (!build || !copy || !out) return;

    build.addEventListener('click', () => {
      const checks = QSA('.academy-checklist input[type="checkbox"]');
      const lines = [];
      lines.push('Checklist QA — Zéro-scroll (Pinapp)');
      lines.push('');
      checks.forEach((c) => {
        const label = c.closest('label')?.textContent?.trim() || '';
        lines.push(`- [${c.checked ? 'x' : ' '}] ${label.replace(/\s+/g, ' ')}`);
      });
      lines.push('');
      lines.push('✅ OK: barre de progression scroll + nav hidden uniquement');
      lines.push('❌ KO: smooth scroll, parallaxe décorative, reveal au scroll');
      out.value = lines.join('\n');
    });

    copy.addEventListener('click', async () => {
      try {
        await navigator.clipboard.writeText(String(out.value || '').trim());
      } catch {
        // ignore
      }
    });
