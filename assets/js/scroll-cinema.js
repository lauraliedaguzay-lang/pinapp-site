  ...
  window.addEventListener(
    ...
    () => {
      ...
      requestAnimationFrame(() => {
        ...
        if (canvas) canvas.style.transform = `translateY(${y * 0.06}px)`;
        orbs.forEach((o) => (o.style.transform = `translateY(${y * -0.03}px)`));
        shapes.forEach(
          (s) => (s.style.transform = `translateY(${y * -0.05}px) rotate(${y * 0.01}deg)`),
        );
        ticking = false;
      });
      ticking = true;
