...
/* Conformité Pinapp "zéro scroll" :
   - Pas d’animations déclenchées au scroll (hors barre de progression gérée ailleurs)
   - Pas de reveal / parallaxe décorative au défilement
...
if (prefersReducedMotion) {
  ...
  /* ZÉRO-SCROLL : pas d’IntersectionObserver. Animations au chargement uniquement. */
  function revealImagesAndAnimateCountsOnLoad() {
    document.querySelectorAll('.img-reveal').forEach((img) => img.classList.add('visible'));
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', revealImagesAndAnimateCountsOnLoad);
  } else {
    revealImagesAndAnimateCountsOnLoad();
  }
}

/* Scroll indicator : on le masque au bout de 4s (pas lié au scroll). */
...
/* ZÉRO-SCROLL : parallaxe décorative interdite. (supprimée) */
